# ASTRION — Engineering reference

The detailed reference that used to make up most of the README: setup in full,
deployment notes, the API, environment variables, operations intelligence, and
workspace access control. The [README](../README.md) is the overview; the
[architecture note](architecture.md) and [security note](SECURITY.md) are the
authoritative design documents.

The *Implementation phases* section is a build history. It records how the
system was assembled and may name module layouts that have since moved on; the
README and `architecture.md` describe the system as it is.
...
shape — lives in [docs/architecture.md](architecture.md). The full
...
[docs/product.md](product.md). The expanded, standalone version of that
...
Design detail lives in [docs/architecture.md](architecture.md).
...
- [Structured-data layer](architecture.md#7-structured-data-layer-phase-2)
  — schema, timestamp handling, provenance.
- [Document / evidence layer](architecture.md#8-document--evidence-layer-phase-3)
  — chunking, source authority, conflict resolution, account scoping.
- [Agent, tools and actions](architecture.md#9-agent-tools-and-actions-phase-4)
  — orchestration, tool boundaries, the deterministic policy engine, and the
  confirmation gate on state-changing actions.
- [Application API and real agent integration](architecture.md#10-application-api-and-real-agent-integration-phase-5)
...
[docs/SECURITY.md](SECURITY.md) for the full list of what is and is not
solved.
...
exist *in this repository*. The [live deployment](../README.md#live-demo) is
...
it yourself, and see [docs/SECURITY.md](SECURITY.md) for where a mail
sender would attach.
...
exist. See [docs/product.md](product.md#roles-and-what-each-may-do).
...
[docs/architecture.md §13](architecture.md#13-deferred-decisions) and
[docs/product.md — Future work](product.md#future-work), ordered by what
...
**Why it's first:** The [live deployment](../README.md#live-demo) is real, but
...
what's asked and does the defensive surfacing [product.md](product.md)
...
[docs/architecture.md §17](architecture.md#17-phase-5--accountable-actions).
